...
────────
  ...
  • IBKR live ticks with conId→symbol O(1) resolution and symbol fallback
  ...
  • Persistence:
      ...
      per-list symbols  → ~/.qullamaggie/.../watchlists/watchlist_{id}.json
...
def _clean_symbol(symbol: Any) -> str:
    """Normalize user-entered/IBKR/Kite-style symbols to the table key."""
    text = str(symbol or "").strip().upper()
    if not text:
        return ""
    # Accept common external formats: SMART:AAPL, NASDAQ:AAPL, AAPL.NASDAQ.
    if ":" in text:
        text = text.split(":")[-1].strip()
    # Keep BRK.B/RDS.A style symbols intact; strip exchange suffixes only when
    # the suffix looks like an exchange name rather than a class separator.
    if "." in text:
        left, right = text.split(".", 1)
        if right in {"SMART", "NYSE", "NASDAQ", "ARCA", "AMEX", "BATS", "NSE", "BSE"}:
            text = left
    return text
...
def _token_from_instrument(inst: Dict[str, Any]) -> int:
    return _safe_int(inst.get("instrument_token") or inst.get("conId") or inst.get("conid") or 0, 0)


def _subscription_payload(symbol: str, inst: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Return a broker-neutral subscription item accepted by MarketDataWorker."""
    inst = dict(inst or {})
    sym = _clean_symbol(inst.get("tradingsymbol") or inst.get("symbol") or symbol)
    token = _token_from_instrument(inst)
    exchange = inst.get("exchange") or inst.get("primaryExchange") or "SMART"
    currency = inst.get("currency") or "USD"
    payload = {
        "tradingsymbol": sym,
        "symbol": sym,
        "exchange": exchange,
        "primaryExchange": inst.get("primaryExchange") or exchange,
        "currency": currency,
        "secType": inst.get("secType") or inst.get("instrument_type") or "STK",
    }
    if token > 0:
...
class TradingTable(QTableWidget):
    ...
    def set_instrument_map(self, instrument_map: Dict[str, Dict]) -> None:
        self._instrument_map = {
            _clean_symbol(k): dict(v)
            for k, v in (instrument_map or {}).items()
            if _clean_symbol(k)
        }
        self._init_data_for_existing(refresh_metadata=True)
        self._repopulate()
    ...
    def add_symbol(self, symbol: str) -> bool:
        ...
        # In IBKR mode the symbol may not be in a local map yet. Accept it
        # immediately and let the market-data worker resolve/qualify it.
        ...
        self._init_symbol_data(symbol_key, refresh_metadata=True)
        self._repopulate()
    ...
    def apply_symbol_snapshot(
        ...
        *,
        ...
        key = self._resolve_symbol_for_instrument_map(symbol) or _clean_symbol(symbol)
        data = self._watchlist_data.get(key)
        if not data:
            return

        price = _safe_float(ltp, 0.0)
        if price > 0:
            data["ltp"] = price
            data["last_price"] = price

        close = _safe_float(prev_close, 0.0)
        if close > 0:
            data["prev_close"] = close

        parsed_volume = _safe_int(volume, 0)
        if parsed_volume > 0:
            data["volume"] = parsed_volume

        token = _safe_int(instrument_token, 0)
        if token > 0:
        ...
        prev = _safe_float(data.get("prev_close"), 0.0)
        cur = _safe_float(data.get("ltp"), 0.0)
        data["change_pct"] = (cur - prev) / prev * 100 if prev > 0 and cur > 0 else 0.0
        self._rebuild_token_map()

        if key in self._symbol_to_row:
            self._dirty.add(key)
    ...
    def remove_symbol(self, symbol: str) -> bool:
        ...
        self._watchlist_data.pop(key, None)
        self._rebuild_token_map()
        self._repopulate()
        self.watchlist_symbols_changed.emit()
        return True
    ...
    def get_all_tokens(self) -> List[int]:
        """Backward-compatible token list for legacy callers."""
        return list(self._token_to_symbol.keys())

    def get_subscription_item(self, symbol: str) -> Optional[Dict[str, Any]]:
        sym = self._resolve_symbol_for_instrument_map(symbol) or _clean_symbol(symbol)
        if not sym or sym not in self._symbols:
            return None
        data = self._watchlist_data.get(sym) or {}
        inst = self._instrument_map.get(sym) or data
        return _subscription_payload(sym, inst)

    def get_subscription_items(self) -> List[Dict[str, Any]]:
        """Return IBKR-friendly subscription payloads for all symbols in this table."""
        items: List[Dict[str, Any]] = []
        for sym in self._symbols:
            item = self.get_subscription_item(sym)
            if item:
                items.append(item)
        return items
    ...
    def update_data(self, ticks: List[Dict]) -> None:
        """Process broker ticks — O(1) token lookup, symbol fallback for IBKR."""
        import time
        if ticks:
        ...
        for tick in ticks or []:
            sym = None
            raw = tick.get("instrument_token") or tick.get("conId") or tick.get("conid")
            token = _safe_int(raw, 0)
            if token > 0:
                sym = self._token_to_symbol.get(token)

            if not sym:
                raw_symbol = tick.get("tradingsymbol") or tick.get("symbol") or tick.get("localSymbol")
                raw_symbol = _clean_symbol(raw_symbol)
                if raw_symbol:
            ...
            ltp = _safe_float(tick.get("last_price") or tick.get("ltp") or tick.get("market_price"), 0.0)
            if ltp > 0:
                data["ltp"] = ltp
                data["last_price"] = ltp

            if token > 0 and not _safe_int(data.get("instrument_token"), 0):
                data["instrument_token"] = token
                data["conId"] = token

            for vf in ("volume_traded", "volume"):
                vol = _safe_int(tick.get(vf), 0)
                if vol > 0:
                    data["volume"] = vol
                    break
            ...
            if isinstance(ohlc, dict):
                prev_close = _safe_float(ohlc.get("close"), 0.0)
            if prev_close <= 0:
                prev_close = _safe_float(tick.get("close") or tick.get("prev_close") or tick.get("previous_close"), 0.0)
            if prev_close > 0:
                data["prev_close"] = prev_close

            prev = _safe_float(data.get("prev_close"), 0.0)
            cur = _safe_float(data.get("ltp"), 0.0)
            tick_pct = tick.get("change_percent") if tick.get("change_percent") is not None else tick.get("change_pct")
            parsed_pct = _safe_float(tick_pct, None) if tick_pct is not None else None
            if parsed_pct is not None:
                data["change_pct"] = parsed_pct
    ...
    def _init_data_for_existing(self, refresh_metadata: bool = False):
        current = set(self._symbols)
        for stale in [sym for sym in self._watchlist_data if sym not in current]:
            self._watchlist_data.pop(stale, None)
        for sym in self._symbols:
            if sym not in self._watchlist_data:
                self._init_symbol_data(sym, refresh_metadata=True)
            elif refresh_metadata:
                self._refresh_symbol_metadata(sym)
        self._rebuild_token_map()

    def _init_symbol_data(self, symbol: str, refresh_metadata: bool = False):
        symbol = _clean_symbol(symbol)
        inst = self._instrument_map.get(symbol, {})
        ohlc = inst.get("ohlc", {}) or {}
        prev = _safe_float(ohlc.get("close") if isinstance(ohlc, dict) else inst.get("prev_close"), 0.0)
        ltp = _safe_float(inst.get("last_price") or inst.get("ltp"), 0.0)
        vol = _safe_int(inst.get("volume"), 0)
        chg = (ltp - prev) / prev * 100 if prev > 0 and ltp > 0 else _safe_float(inst.get("change_percent") or inst.get("change_pct"), 0.0)
        token = _token_from_instrument(inst)
        ...
        self._watchlist_data[symbol] = {
            ...
            "conId": token or None,
            "exchange": inst.get("exchange") or inst.get("primaryExchange") or "SMART",
            "primaryExchange": inst.get("primaryExchange") or inst.get("exchange") or "SMART",
            "currency": inst.get("currency") or "USD",
            "ltp": ltp,
            "last_price": ltp,
            "volume": vol,
        ...
        if refresh_metadata:
            self._refresh_symbol_metadata(symbol)

    def _refresh_symbol_metadata(self, symbol: str) -> None:
        """Merge newly resolved IBKR contract fields without wiping live prices."""
        ...
        if token > 0:
            data["instrument_token"] = token
            data["conId"] = token
        for key in ("exchange", "primaryExchange", "currency", "secType", "name"):
            if inst.get(key):
                data[key] = inst.get(key)
        if _safe_float(data.get("ltp"), 0.0) <= 0:
            ltp = _safe_float(inst.get("last_price") or inst.get("ltp"), 0.0)
            if ltp > 0:
                data["ltp"] = ltp
                data["last_price"] = ltp
        if _safe_float(data.get("prev_close"), 0.0) <= 0:
            ohlc = inst.get("ohlc") or {}
            prev = _safe_float(ohlc.get("close") if isinstance(ohlc, dict) else inst.get("prev_close"), 0.0)
            if prev > 0:
...
class TabbedWatchlistWidget(QWidget):
    ...
    quote_snapshot_requested = Signal(list)
    place_order_requested = Signal(dict)
    advanced_buy_order_requested = Signal(str)
    advanced_sell_order_requested = Signal(str)
    bracket_order_requested = Signal(str)
    ...
    def _refresh_symbol_snapshot(self, table: TradingTable, symbol: str) -> None:
        """Request a non-blocking quote snapshot for closed-market LTP/%Chg.

        The old path called broker quote/historical APIs from the UI thread. In
        IBKR mode that can freeze the watchlist while TWS qualifies contracts.
        Now the main window routes this request to MarketDataWorker, which emits
        the same tick format as live streaming.
        """
        item = table.get_subscription_item(symbol) if table else None
        if item:
            self.quote_snapshot_requested.emit([item])
    ...
    def get_all_watchlist_tokens(self) -> List[int]:
        """Return tokens from all watchlists (legacy diagnostics)."""
        tokens = []
        for table in self._tables.values():
    ...
    def get_subscription_items(self) -> List[Dict[str, Any]]:
        ...
        table = self._current_table()
        return table.get_subscription_items() if table else []

    def get_all_subscription_items(self) -> List[Dict[str, Any]]:
        """Return subscription items for every watchlist, including symbols without conId yet."""
        seen = set()
        items: List[Dict[str, Any]] = []
        for table in self._tables.values():
            for item in table.get_subscription_items():
                token = item.get("instrument_token") or item.get("conId")
                key = str(token) if token else _clean_symbol(item.get("tradingsymbol") or item.get("symbol"))
                if key and key not in seen:
                    seen.add(key)
                    items.append(item)
        return items

    def apply_quote_snapshots(self, ticks: List[Dict[str, Any]]) -> None:
        """Apply snapshot ticks to all tables; used by the worker's closed-market path."""
        self.update_data(ticks or [])
    ...
    def _subscribe_all_tokens(self):
        ...
        if items:
            self.subscribe_tokens_requested.emit(items)
            self.quote_snapshot_requested.emit(items)
